...
- `[tool.uv] constraint-dependencies = ["mcp<2"]` を追加し、mcp を **1.x 系に固定**した。
  ...
  - **理由**: 2.0.0 で `mcp.server.fastmcp` が `mcp.server.mcpserver` へ改名され、
    `src/nene2/mcp/server.py` が `ModuleNotFoundError` で壊れる（実測済み）。
  ...
  - 🔑 `constraint-dependencies` は `uv.lock` の解決にのみ適用され、
    **`requires-dist` には漏れない**。実測で確認済み（`mcp` の `requires-dist` は
...
- `msgpack` は `cachecontrol` ← `pip-audit[filecache]` 経由の **dev 依存のみ**で、
  配布物には含まれない。利用者には影響しない。

CI を push から切り離し、週次で外界の変化（上流 advisory）を検知できるようにした。

### Added
- `.github/workflows/ci.yml` に **週次 `schedule`**（`cron: '30 0 * * 1'` = 月曜 09:30 JST）と
  **`workflow_dispatch`** を追加。検知したいのはコードの変化ではなく**外界の変化**である。
  ...
  `pip-audit` で正確に測る。
- **`concurrency` グループ**を追加し、`${{ github.event_name }}` を**含めた**。
...
- 🔴 **既知の限界**: public リポジトリの scheduled workflow は、リポジトリの活動が
  **60 日間**無いと GitHub により自動的に無効化される。本リポジトリの空白は **74 日**で
...
- 🔴 **訂正**: v1.8.167 で mcp 2.0 移行を「`src/nene2/mcp/` と `src/example/mcp.py` の
...
### Security
- `pip-audit` が検出していた **22 件（出力 27 行・重複含む）/ 8 パッケージ** を解消。
  コードは 2026-05-30 以降変更されておらず、**上流 advisory データ側の更新**によって
  据え置きの `uv.lock` が相対的に古くなったことが原因。

  | パッケージ | 変更 | 解消した ID |
  |---|---|---|
  | pyjwt | 2.12.1 → 2.13.0 | PYSEC-2026-175 / 176 / 177 / 178 / 179 |
  | cryptography | 48.0.0 → 50.0.0 | PYSEC-2026-3552 / 3553 / 3554・GHSA-537c-gmf6-5ccf |
  | starlette | 1.0.1 → 1.6.0 | PYSEC-2026-248 / 249 / 2280 / 2281 |
  | mcp | 1.27.1 → 1.29.0 | PYSEC-2026-3481 / 3482 / 3483 |
  | python-multipart | 0.0.29 → 0.0.32 | PYSEC-2026-3036 / 3037 / 3040 |
  | msgpack | 1.1.2 → 1.2.1 | PYSEC-2026-3625 |
  | pydantic-settings | 2.14.1 → 2.15.0 | GHSA-4xgf-cpjx-pc3j |
  | pip | 26.1.1 → 26.2.1 | PYSEC-2026-196 |

- **新規の `--ignore-vuln` 追加はゼロ**。CI の除外は既存の `PYSEC-2025-183` 1 件のみ。

### Changed
- `pyproject.toml` の**依存宣言は無変更**（すべて下限のみのため `uv.lock` の解決だけで到達。
  変更は `version` の 1.8.166 → 1.8.167 のみ）。したがって PyPI 配布物の依存要件は変わらない。
- `mcp` は **1.x 系最新の 1.29.0 で止めた**。2.0.0 は `mcp.server.fastmcp` モジュールを
  削除しているため、別タスクとして分離した。advisory の要求は 1.28.1 以上であり 1.29.0 で充足する。
  > 🔴 **本項の当初記述を v1.8.168 で訂正**: ここに「`src/nene2/mcp/` と `src/example/mcp.py` の
