...
最終更新: 2026-05-29
現状: **v1.8.128 / FT250（decimal）完了 / CI グリーン**

---

## 状態サマリー

FT250（decimal — Decimal / 精度 / 丸めモード）完了。診断なし（250 % 3 = 1）・ペンテストなし（250 % 4 = 2）。
金額は float ではなく**文字列で受信**し `Decimal("0.1")` で生成（0.1+0.2=0.30 を厳密化）。
丸めモードは許可リスト（half_up/half_even、既定 HALF_EVEN の罠に注意）、NaN/Infinity は is_finite() で拒否。
**サンドボックス 9 tests**、フレームワーク本体 466 tests 据え置き。フィールドトライアルループは FT251 以降も継続中。
...
**実施済み**: FT1〜FT250（全 250 件）

索引: [`docs/field-trials/INDEX.md`](../field-trials/INDEX.md)

**次のアクション**:
- FT251 を開始（251 % 3 = 2 → セキュリティ診断なし、251 % 4 = 3 → クラッカーペンテストなし）
- テーマ候補: `fractions` モジュール（Fraction / 有理数演算 / limit_denominator）
...
| 優先度 | Issue | タスク | 種別 |
|---|---|---|---|
| 高 | — | FT251 実施（fractions、診断・ペンテストなし） | FT |
