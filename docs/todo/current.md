...
最終更新: 2026-05-29
現状: **v1.8.108 / FT230（difflib）完了 / CI グリーン**

---

## 状態サマリー

FT230（difflib — unified_diff / SequenceMatcher / get_close_matches）完了。診断なし（230 % 3 = 2）・ペンテストなし（230 % 4 = 2）。
`unified_diff` は文字列ではなく `splitlines(keepends=True)` の行リストを取る点に注意。
`SequenceMatcher.ratio()` は O(n^2) のため入力長を 20,000 文字に制限し CPU DoS を防止。候補数も上限化。
**サンドボックス 7 tests**、フレームワーク本体 466 tests 据え置き。フィールドトライアルループは FT231 以降も継続中。
...
**実施済み**: FT1〜FT230（全 230 件）

索引: [`docs/field-trials/INDEX.md`](../field-trials/INDEX.md)

**次のアクション**:
- FT231 を開始（231 % 3 = 0 → **セキュリティ診断あり**、231 % 4 = 3 → クラッカーペンテストなし）
- テーマ候補: `shlex` モジュール（split / quote / join・シェルインジェクション対策）
...
| 優先度 | Issue | タスク | 種別 |
|---|---|---|---|
| 高 | — | FT231 実施（shlex、セキュリティ診断あり） | FT |
