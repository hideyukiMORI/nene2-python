...
最終更新: 2026-05-29
現状: **v1.8.142 / FT264（ast.literal_eval）完了 / CI グリーン**

---

## 状態サマリー

FT264（ast.literal_eval — eval の安全な代替）完了。**セキュリティ診断あり（264 % 3 = 0）＋クラッカーペンテストあり（264 % 4 = 0）両方合格**。
literal_eval は Python リテラルのみ解析しコード実行しない（eval/exec は CLAUDE.md 禁止）。
__import__/open/__subclasses__/globals/exec/lambda/内包表記の 10 コードインジェクションを全て 422、深いネスト(2000→3ms)・巨大整数 DoS も遮断。tuple/set/complex は JSON 正規化。
**サンドボックス 8 tests**、フレームワーク本体 466 tests 据え置き。フィールドトライアルループは FT265 以降も継続中。
...
**実施済み**: FT1〜FT264（全 264 件）

索引: [`docs/field-trials/INDEX.md`](../field-trials/INDEX.md)

**次のアクション**:
- FT265 を開始（265 % 3 = 1 → セキュリティ診断なし、265 % 4 = 1 → クラッカーペンテストなし）
- テーマ候補: `zoneinfo` モジュール（IANA タイムゾーン変換・ZoneInfo）
...
| 優先度 | Issue | タスク | 種別 |
|---|---|---|---|
| 高 | — | FT265 実施（zoneinfo、診断・ペンテストなし） | FT |
