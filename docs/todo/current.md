...
最終更新: 2026-05-29
現状: **v1.8.143 / FT265（zoneinfo）完了 / CI グリーン**

---

## 状態サマリー

FT265（zoneinfo — IANA タイムゾーン変換）完了。診断なし（265 % 3 = 1）・ペンテストなし（265 % 4 = 1）。
`available_timezones()` で tz 名を事前検証、naive datetime には from_tz を付与してから astimezone。DST は IANA DB で自動処理（NY 7月 -04:00 / 冬 -05:00）。
pytz より zoneinfo が推奨（fold 対応）。保存は UTC aware・変換は境界で。
**サンドボックス 5 tests**、フレームワーク本体 466 tests 据え置き。フィールドトライアルループは FT266 以降も継続中。
...
**実施済み**: FT1〜FT265（全 265 件）

索引: [`docs/field-trials/INDEX.md`](../field-trials/INDEX.md)

**次のアクション**:
- FT266 を開始（266 % 3 = 2 → セキュリティ診断なし、266 % 4 = 2 → クラッカーペンテストなし）
- テーマ候補: `types` モジュール（MappingProxyType による読み取り専用ビュー）
...
| 優先度 | Issue | タスク | 種別 |
|---|---|---|---|
| 高 | — | FT266 実施（types、診断・ペンテストなし） | FT |
