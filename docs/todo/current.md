...
最終更新: 2026-05-29
現状: **v1.8.158 / FT280（re）完了 / CI グリーン**

---

## 状態サマリー

FT280（re — ReDoS 対策）完了。診断なし（280 % 3 = 1）・**クラッカーペンテストあり（280 % 4 = 0）合格**。
ユーザーに正規表現パターンを供給させず（固定許可リスト）、線形（ReDoS 非脆弱）パターン + 入力長制限で照合。
ペンテスト: 線形パターンは 1000 文字の敵対入力でも 3〜29ms、パターン注入は 422。対比で脆弱 `(a+)+$` は 28 文字に 9.7 秒（ReDoS）かかることを実証。FT232 と一貫。
**サンドボックス 6 tests**、フレームワーク本体 466 tests 据え置き。フィールドトライアルループは FT281 以降も継続中。
...
**実施済み**: FT1〜FT280（全 280 件）

索引: [`docs/field-trials/INDEX.md`](../field-trials/INDEX.md)

**次のアクション**:
- FT281 を開始（281 % 3 = 2 → セキュリティ診断なし、281 % 4 = 1 → クラッカーペンテストなし）
- テーマ候補: `math` モジュール（isclose / gcd / factorial の過大入力ガード）
