...
最終更新: 2026-05-29
現状: **v1.8.133 / FT255（sqlite3）完了 / CI グリーン**

---

## 状態サマリー

FT255（sqlite3 — パラメータ化クエリ vs SQL インジェクション）完了。**セキュリティ診断あり（255 % 3 = 0）合格**・ペンテストなし（255 % 4 = 3）。
`?` プレースホルダで注入文字列を「リテラルデータ」化し、`execute` の単文制約と合わせて SQL インジェクションを完全遮断。
診断: OR バイパス・DROP/DELETE・UNION・LIKE ワイルドカードの 9 種をすべて無効化（matches=[]・table_intact=True）。CLAUDE.md の SQL ポリシーを実証。
**サンドボックス 5 tests**、フレームワーク本体 466 tests 据え置き。フィールドトライアルループは FT256 以降も継続中。
...
**実施済み**: FT1〜FT255（全 255 件）

索引: [`docs/field-trials/INDEX.md`](../field-trials/INDEX.md)

**次のアクション**:
- FT256 を開始（256 % 3 = 1 → セキュリティ診断なし、256 % 4 = 0 → **クラッカーペンテストあり**）
- テーマ候補: `email.message` モジュール（ヘッダーインジェクション / CRLF）
...
| 優先度 | Issue | タスク | 種別 |
|---|---|---|---|
| 高 | — | FT256 実施（email、クラッカーペンテストあり） | FT |
