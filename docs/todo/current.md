...
最終更新: 2026-05-29
現状: **v1.8.127 / FT249（hmac）完了 / CI グリーン**

---

## 状態サマリー

FT249（hmac — new / compare_digest）完了。**セキュリティ診断あり（249 % 3 = 0）合格**・ペンテストなし（249 % 4 = 1）。
HMAC-SHA256（md5/sha1 不使用）+ `compare_digest` で Webhook 署名検証パターンを実装。`==` はコメントのみ。
改ざん（メッセージ・鍵・署名切り詰め・1 文字反転）を全検出、不正署名は例外を漏らさず valid=False。FT222（hashlib）と相補。
**サンドボックス 7 tests**、フレームワーク本体 466 tests 据え置き。フィールドトライアルループは FT250 以降も継続中。
...
**実施済み**: FT1〜FT249（全 249 件）

索引: [`docs/field-trials/INDEX.md`](../field-trials/INDEX.md)

**次のアクション**:
- FT250 を開始（250 % 3 = 1 → セキュリティ診断なし、250 % 4 = 2 → クラッカーペンテストなし）
- テーマ候補: `decimal` モジュール（Decimal / 精度 / 丸めモード・金額計算）
...
| 優先度 | Issue | タスク | 種別 |
|---|---|---|---|
| 高 | — | FT250 実施（decimal、診断・ペンテストなし） | FT |
